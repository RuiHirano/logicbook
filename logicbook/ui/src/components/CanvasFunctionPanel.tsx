...
const CanvasFunctionPanel: React.FC<Props> = ({ logic, onExecuteTest, onExecuteAllTest }) => {
  logic.tests = []
  ...
  return (
    <div>
      ...
      <Paper>
        ...
        >
          {logic.funcs.length === 0 && <Typography variant="body2" style={{ textAlign: 'center' }}>No Functions</Typography>}
          {logic.funcs.map((func: any, i: number) => (
            <div key={i}>
              <ListItemButton onClick={() => handleClick(i)}>
                <ListItemIcon>
                  {func.status === "success" && <SuccessIcon style={{ color: "green" }} />}
                  {func.status === "failure" && <FailureIcon style={{ color: "red" }} />}
                  {func.status === "unknown" && <UnknownIcon style={{ color: "blue" }} />}
                </ListItemIcon>
              ...
              <Collapse in={open[i]} timeout="auto" unmountOnExit>
                <List component="div" disablePadding>
                  <ListItem sx={{ pl: 4 }}>
                    ...
                    <ListItemText primary={func.path} />
                  </ListItem>
                  <ListItem sx={{ pl: 4 }}>
                    <ListItemIcon>
                      <TimeIcon />
                    </ListItemIcon>
                    <ListItemText primary={moment(func.latest_run_time).format("YYYY/MM/DD HH:mm:ss")} />
                  </ListItem>
                </List>
                ...
                <div style={{ marginLeft: 40, marginRight: 40 }}>
                  ...
                  <div style={{ height: 200 }}>
                    <LogTerminal text={loading[i] ? "" : func.code} loading={loading[i]} />
                  </div>
                </div>
                <div style={{ marginLeft: 40, marginRight: 40 }}>
                  <Typography style={{ margin: 5, fontWeight: 'bold' }} variant="body1">Result</Typography>
                  <div style={{ height: 200 }}>
                    <LogTerminal text={loading[i] ? "" : func.result} loading={loading[i]} />
                  </div>
                </div>
...
export default CanvasFunctionPanel;
